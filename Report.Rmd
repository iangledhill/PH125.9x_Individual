...
The DoT record more than 500,000 domestic flights per months. The period covered by this project contains `r format_number(nrow(flights))` data points. Each data point has `r length(flights)` measurements.
...
Note that the `r format_percent(mean(flights$DEP_STATUS == "UNKNOWN"))` percentage of flights (equal to `r nrow(flights[flights$DEP_STATUS == "UNKNOWN"])` flights) were not cancelled but have no depature time recorded. 
...
CART (_classification and regression trees_) has been adopted to build a predictive model.
...
rpart.plot(train_rpart$finalModel, box.palette="Blues")
```
